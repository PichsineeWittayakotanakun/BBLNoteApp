    ...
    async (
      ...
      await fetch(`${Endpoint}/comments/${commentId}`, {
        ...
        .then(async res => {
          if (res.status !== 200) {
            console.error(
              'APIs Response Error getCommentByIdAtomService',
              await res.json(),
            );
        ...
        .catch(err => {
          console.log('Error fetching getCommentByIdAtomService', err);
          set(this._isLoading, false);
          set(this._isError, true);
          return;
        });
...
const getCommentByIdAtomService = new GetCommentByIdAtomService();

export default getCommentByIdAtomService;
